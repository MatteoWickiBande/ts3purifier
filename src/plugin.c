...
char isAllowed(uint64 serverConnectionHandlerID, uint64 id) {
	...
	unsigned int ret = ts3Functions.getClientVariableAsString(serverConnectionHandlerID, (anyID)id, CLIENT_PLATFORM, &os);

	if (ret!= ERROR_ok || strlen(os)==0) {
		ts3Functions.requestClientVariables(serverConnectionHandlerID, (anyID)id, NULL);
		return 2;
	}

	if (strcmp("Linux", os) == 0) {
		return 1;
	}
...
const char* ts3plugin_author() {
	...
	return "Matteo Signer <matteo.signer@gmail.com>";
}
...
void ts3plugin_infoData(uint64 serverConnectionHandlerID, uint64 id, enum PluginItemType type, char** data) {
	if (type == PLUGIN_CLIENT) {
		char valid = isAllowed(serverConnectionHandlerID, id);
...
void ts3plugin_onUpdateClientEvent(uint64 serverConnectionHandlerID, anyID clientID, anyID invokerID, const char* invokerName, const char* invokerUniqueIdentifier) {
	...
	uint64 otherChannel;
	ts3Functions.getChannelOfClient(serverConnectionHandlerID, clientID, &otherChannel);
	if(myChannel != otherChannel)
		return;
...
void ts3plugin_onClientMoveEvent(uint64 serverConnectionHandlerID, anyID clientID, uint64 oldChannelID, uint64 newChannelID, int visibility, const char* moveMessage) {
	...
	if(clientID == me)
		return;
	printf("User tried to enter\n");
	uint64 myChannel;
	ts3Functions.getChannelOfClient(serverConnectionHandlerID, me, &myChannel);
	if(myChannel != newChannelID)
		return;
	char allowed = isAllowed(serverConnectionHandlerID, clientID);
	if(allowed==0) {
		ts3Functions.requestClientKickFromChannel(serverConnectionHandlerID, clientID, kickMsg, NULL);
		...
		printf("Requesting client infos...\n");
	} else {
		printf("Accepted!\n");
	}
